...
var SearchList = function SearchList(_a) {
  var route = _a.route; //param config
  ...
  var title = route !== undefined ? route.params.title : null; //user data
  ...
  var onCreateRequest = function onCreateRequest(item_id, item_name) {
    ...
    if (list_type === 'sub-service') {
      ...
      filteredArrays = allList.filter(function (_a) {
        var nama_bank = _a.nama_bank,
            request_client = _a.request_client;
        return nama_bank.toLowerCase().includes(query.toLowerCase()) || request_client.toLowerCase().includes(query.toLowerCase());
      });
    } else if (list_type === 'bank' || list_type === 'appointment_bank') {
      filteredArrays = allList.filter(function (_a) {
        var nama_bank = _a.nama_bank;
  ...
  react_1["default"].useEffect(function () {
    setRefreshing(true);
    populateData(true); // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  }, "Pilih ", title))), react_1["default"].createElement(react_native_1.View, {
  ...
  }), searchList.length > 0 ? react_1["default"].createElement(react_native_1.FlatList, {
    fadingEdgeLength: 30,
    data: searchList,
